...
const { totalVehiclesByBridge } = require('../src/controllers/totalVehiclesByBridge');

describe('totalVehiclesByBridge endpoint', () => {
  ...
  it('should return total vehicles passed through the specified bridge and date', async () => {
    ...
    const res = {
      status: (statusCode) => ({
        json: (data) => {
          ...
          chai.expect(data.tollRecords).to.be.an('array');
        },
      }),
    };

    await totalVehiclesByBridge(req, res);
  });
